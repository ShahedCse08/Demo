...
namespace Demo.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
       ...
       //     CompanyEmployeesDto result = new CompanyEmployeesDto();
       ...
       // }
        ...
        public async Task<IActionResult> GetEmployeesForCompanyWithParentChildCombination(Guid companyId, [FromQuery] EmployeeParameters employeeParameters)
        {
            if (!employeeParameters.ValidAgeRange)
                return BadRequest("Max age can't be less than min age.");

            var company = await _repository.Company.GetCompanyWithChildListAsync(companyId, trackChanges: false);
            if (company == null)
            {
                _logger.LogInfo($"Company with id: {companyId} doesn't exist in the database.");
                return NotFound();
            ...
            var companyDto = _mapper.Map<CompanyEmployeesDto>(company);
